...
scriptContext:SetCallback(function()
    local script = "-- This script was generated by Hydroxide(modified by perhaps#0309)\n\n"
    ...
    if #selected.args == 0 then
        ...
        for i = 1, #selectedArgs do
            ...
            elseif valueType == "function" then 
                local functionInfo = islclosure(v) and getinfo(v) or {name="cclosure"}
                local constantsInfo = islclosure(v) and getconstants(v) or {}
                local upvaluesInfo = getupvalues(v) or {}
                
                script = script.."[[\n".."   Function: %s\n":format(functionInfo.name)
                if functionInfo.nparams > 0 then
                    script = script.."    Parameters: %s\n":format(table.concat(functionInfo.params, ", "))
                end

                if #constantsInfo > 0 then
                    script = script.."    Constants: %s\n":format(tableToString(constantsInfo))
                end

                if #upvaluesInfo > 0 then
                    script = script.."    Upvalues: %s\n":format(tableToString(upvaluesInfo))
                end

                -- check if there are more functions left in the args
                local found = false  
                for i1, v1 in pairs(selectedArgs) do
                    if type(v) == "function" and i1 > i then
                        found = true 
                    end
                end
            ...
            local realvalue = selectedArgs[i]

            local varName = ""
            if valueType == "string" and #realvalue > 1 and realvalue ~= "" and realvalue ~= " " and realvalue ~= "\n" then 
                varName = "%s%d":format(variableName, i)
            elseif valueType == "function" then
                local functionInfo = isclosure(realvalue) and getinfo(realvalue) or {name="cclosure"}
                local funcName = functionInfo.name
                funcName = funcName ~= "" and funcName ~= " " and funcName ~= "\n" and funcName or "UnnamedFunction"
                varName = "%s%d":format(variableName, i)
            else
